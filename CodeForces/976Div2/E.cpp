...
void solve() {
    ...
    for(int i=0;i<1024;i++) for(int j=0;j<2;j++) dp[j][i] = 0;
    dp[0][0] = 1;
    ...
    for(int i=0;i<n;i++) {
        ...
        for(int j=0;j<1024;j++) {
            dp[nxt][j^a[i]] += dp[cur][j]*p[i];
            dp[nxt][j] += dp[cur][j]*(den-p[i]);
        }
        for(int j=0;j<1024;j++) dp[cur][j] = 0;
    }
    ...
    for(int j=0;j<1024;j++) dp[x][j] *= den;

    mi ans = 0;
    for(int j=0;j<1024;j++) ans += dp[x][j]*j*j;
