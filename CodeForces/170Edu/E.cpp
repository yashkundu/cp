...
void solve() {
    ...
    for(int i=1;i<=n-1;i++) {
        for(int j=0;j<=m;j++) {
            for(int k=0;k<=min(j, m);k+=2) {
                dp[i][j] += dp[i-1][j-k]*cnt[m][(m-k)/2];
            }
        }
    }
    ...
    // debug(cnt[2][0].v);
    // debug(dp[1][2].v);
    // debug(dp[1][0].v, dp[1][1].v, dp[1][2].v);

    for(int i=0;i<=m/2;i++) {
        ans += cnt[m][i]*dp[n-1][m-2*i];
        // debug(i, cnt[m][i].v, dp[n-1][m-2*i].v, ans.v);
    }
