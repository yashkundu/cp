...
mi fact[2*MAXN], invfact[2*MAXN];

void init() {
    fact[0] = 1;
    for(int i=1;i<2*MAXN;i++) fact[i] = fact[i-1]*i;
    invfact[2*MAXN-1] = inv(fact[2*MAXN-1]);
    for(int i=2*MAXN-2;i>=0;i--) invfact[i] = invfact[i+1]*(i+1);
}
