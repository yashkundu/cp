...
void solve() {
    ...
    for(int i=(1<<(n-1))-1;i>0;i--) {
        ...
        if(repr[left]<repr[right]) repr[i] = s[i] + repr[left] + repr[right];
        else repr[i] = s[i] + repr[right] + repr[left];

        if(repr[left]==repr[right]) dp[i] = dp[left]*dp[right];
        else dp[i] = dp[left]*dp[right]*2;


        // debug(i, dp[i].v, repr[left], repr[right]);
    }
